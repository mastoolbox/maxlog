...
const (
	...
	// Labeling symbols - Nerd Font
	NFPrefixLabel = "\ue0b6"
	NFSuffixLabel = "\ue0b4"

	// Labeling symbols - ASCII
	ASCPrefixLabel = ""
	ASCSuffixLabel = ""
)
...
//
// This function reads the environment variable "MAXLOG_USE_NERDFONT" and parses its value
// as a boolean. If the variable is set to a truthy value (e.g., "true", "1"), it returns true.
// Otherwise, it returns false.
//
// Returns:
//
//	bool - True if Nerd Font symbols are enabled, false otherwise.
func UseNerdFont() bool {
	useNF, _ := strconv.ParseBool(GetEnv("MAXLOG_USE_NERDFONT", "false"))
	return useNF
}
...
func SetLabel(text, oldKey, newKey, color, bgColor string) string {
	prefix, suffix := ASCPrefixLabel, ASCSuffixLabel
	if useNerdFont {
		prefix, suffix = NFPrefixLabel, NFSuffixLabel
	}
	return strings.Replace(text, oldKey, color+prefix+Reset+bgColor+newKey+Reset+color+suffix+Reset, 1)
}
